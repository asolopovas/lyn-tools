...
go-quality:
    ...
    go test ./...
    go vet ./...
    staticcheck_bin="$(command -v staticcheck || true)"; test -n "$staticcheck_bin" || staticcheck_bin="$(go env GOPATH)/bin/staticcheck"; test -x "$staticcheck_bin" || { echo "staticcheck is required: go install honnef.co/go/tools/cmd/staticcheck@latest" >&2; exit 1; }; "$staticcheck_bin" ./...
...
go-quality-windows:
    ...
    go test ./...
    go vet ./...
    $staticcheck = (Get-Command staticcheck -ErrorAction SilentlyContinue | Select-Object -First 1).Source; if (-not $staticcheck) { $staticcheckPath = Join-Path (go env GOPATH) "bin/staticcheck.exe"; if (Test-Path -LiteralPath $staticcheckPath) { $staticcheck = $staticcheckPath } }; if (-not $staticcheck) { Write-Error "staticcheck is required: go install honnef.co/go/tools/cmd/staticcheck@latest" }; & $staticcheck ./...
