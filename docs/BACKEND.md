...
- Windows packaged apps are discovered through the registered Start-app list because standard users cannot enumerate `%ProgramFiles%\WindowsApps`. App icons resolve through `App.Icon`; packaged apps (`shell:AppsFolder\…`) resolve through the Windows shell namespace, while filesystem apps use `SHGetFileInfo`. Resolved icons are validated and atomically cached as PNG under the cache `icons` dir, duplicate frontend loads are coalesced, and failed image decoding falls back to an app glyph.
