...
- Windows packaged apps are discovered through the registered Start-app list because standard users cannot enumerate `%ProgramFiles%\WindowsApps`. App icons resolve through `App.Icon`; packaged apps (`shell:AppsFolder\…`) resolve through the Windows shell namespace, while filesystem apps use `SHGetFileInfo`. Resolved icons cache to PNG under the cache `icons` dir, and the frontend uses an app glyph when native icon resolution fails.
