...
- **uiAccess (release only):** the hook fires over admin-owned windows without elevating Lyn, via a `uiAccess="true"` manifest. Windows grants `uiAccess` only to a signed binary in a secure location, so it is a coupled packaging unit: `scripts/package-windows.ps1` stamps [`build/windows/wails.exe.uiaccess.manifest`](../build/windows/wails.exe.uiaccess.manifest) with `mt.exe`, signs with `scripts/dev-sign.ps1`, and the NSIS installer ([`build/windows/installer.nsi`](../build/windows/installer.nsi)) installs per-machine under `%ProgramFiles%\Lyn`. The dev manifest stays `asInvoker` so unsigned `just dev`/`just build` keep launching. One-time cert: `just dev-sign-setup` (self-signed for dev; a real cert sets `LYN_SIGN_TIMESTAMP_URL` and shares subject `Lyn Code Signing`).
