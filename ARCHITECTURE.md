...
- Ranking order: match quality, then kind tier, then usage count, last launch time, name, path. Kind tiers (`kindRank`): `.code-workspace` first, then everything else (local projects, apps, WSL/non-SSH recents), then VS Code SSH-remote entries (`vscode-remote://ssh-remote+`), then system commands last. So workspaces and WSL/local results always sort above SSH connections, and an empty query lists workspaces first and power commands last.
- Log Out / Restart / Shut Down are virtual `system-command` results (`lyn:system:{logout,restart,shutdown}`) injected in-memory by `indexProjects` (alongside live recents) so they ride every read path but never persist to SQLite; the store open still purges any stale `system-command` rows as defense-in-depth. Launch maps them to the real OS command (`shutdown.exe` on Windows, `systemctl`/`loginctl` on Linux, `osascript` on macOS); the Windows path bypasses `ShellExecute` so `shutdown.exe` runs hidden via the normal background-process launch.
...
- Ignored folders include build/dependency dirs (`node_modules`, `vendor`, `dist`, `wp-includes`, `uploads`, ...) and versioned package-store entries (any `@<digit>` in the name, e.g. bun/pnpm cache dirs like `accepts@1.3.8`), so third-party packages are never indexed as projects and large content trees are not traversed.
- Detecting a project does not stop the scan inside it: traversal continues (skipping ignored folders) to collect `.code-workspace` files nested in the project (e.g. `site/wp-content/themes/<theme>/x.code-workspace`), but nested directory project-markers are not added as separate projects. Without this, workspaces living inside WordPress/Laravel sites were never indexed because detection halted at the site root.
...
- `Open in Code` retargets WordPress projects before launching (`App.resolveLaunchTarget`): it opens the site's `.code-workspace` if one is indexed under the project, otherwise the most recently modified `wp-content/themes/<theme>` folder, and never the site root. Laravel, Go, and other kinds open their root unchanged. Usage is still recorded against the project the user selected, not the retargeted path.
- Remote (WSL/SSH) projects open in VS Code with `--folder-uri vscode-remote://<authority><path>`, not `--remote <authority> <path>`. The `--remote <path>` form opens an empty window (the path is ignored), so the requested project never loads; the folder-uri form is the same encoding VS Code records in its own recents. `.code-workspace` targets use `--file-uri`.
- Launched processes must never inherit Lyn's elevation. When Lyn runs elevated, `launch` reparents the child to the shell (Explorer) process via `PROC_THREAD_ATTRIBUTE_PARENT_PROCESS` so it inherits the shell's medium-integrity token; only `run-admin`/`run-user` keep the explicit `runAs`/`runAsUser` request. VS Code's GUI exe does not survive being reparented directly (Electron relaunch exits), so the elevated `code` path is routed through its CLI (`cmd /c code …`) which spawns the window itself and lands at medium integrity. Non-elevated Lyn keeps the direct `exec`/`ShellExecute` path unchanged.
- `run-admin` launches route through a lazily spawned elevated helper (`lyn --elevated-helper=<pipe>`) so repeated admin launches reuse one UAC consent instead of prompting per launch. The non-elevated UI owns the named-pipe server (medium integrity, security descriptor scoped to the current user SID) and the elevated helper connects as client, so the high-to-medium write avoids the mandatory-integrity no-write-up block. The protocol is newline-delimited JSON (`lyn/elevation_ipc.go`); the helper only executes whitelisted `run-admin` requests and starts the target with `ShellExecute open` so it inherits the helper's elevated token. If the helper cannot start or the pipe fails, `App.Launch` falls back to the existing per-launch `ShellExecuteW runAs`. The helper exits when the UI closes the pipe (shutdown or crash), so no elevated process is orphaned.
...
- The Linux global hotkey (`hotkey/hotkeys_linux.go`) grabs every Num Lock/Caps Lock mask combination of the binding, not just the bare modifier. X11 reports the active lock masks in key events and the underlying `XGrabKey` matches an exact modifier mask, so a bare `Super+D` grab silently never fires while Num Lock or Caps Lock is on. Each lock variant is a separate grab routed to the same handler; a stale conflicting grab from another client (e.g. a window manager still holding `mod4+d` after its config changed without a restart) makes the underlying C grab abort the process, so the WM must own no overlapping binding.
