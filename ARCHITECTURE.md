...
- Ranking order: match quality, then `.code-workspace` entries before other kinds, then usage count, last launch time, name, path. Workspaces sort above folders within a match group, so an empty query lists workspaces first.
